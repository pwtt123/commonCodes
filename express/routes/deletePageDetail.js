...
//  删除 组件明细 记录
router.post(publicPath+'deletePageDetail.do', function (req, res, next) {
    try{
        var zTemp={};
        var zOptions={};
        var zTargetUserIDs;
        ...
        //筛选传入的数据
        zTemp["原数据"]= _.pick(req.body,"userID","pageDetailID","pageID");
        ...
        //配置文件地址存在
        if(settings.interfaces.deletePageDetail){
            try{


                //调用外部接口
                zInterface.execExternalInterface(
                    settings.interfaces.deletePageDetail,
                    zTemp["xInput数据"],
                    zTemp["xOptions数据"],
                    function(err,data){
