...
// 创建 组件明细 记录
router.post(publicPath+'createPageDetail.do', function (req, res, next) {
    try{
        var zTemp={};
        ...
        //限制传入的数据
        zTemp["原数据"]= _.pick(req.body,"userID","pageID","navItemID","indent","compoHTMLTag",
            "orderID","isExpanded","compoProps","optionGroupName","_ownerID");
        ...
        //配置文件地址存在
        if(settings.interfaces.createPageDetail){
            try{


                //调用外部接口
                zInterface.execExternalInterface(
                    settings.interfaces.createPageDetail,
                    zTemp["xInput数据"],
                    zTemp["xOptions数据"],
                    function(err,data){
